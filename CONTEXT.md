...
- **Issues phase** (default on start): each cycle audits ALL open GitHub issues into a priority + dependency-ordered queue (`.loop/queue.json`, built by `scripts/issue-queue.ts`), then works the top automatable dataset issue from `scripts/dataset-fixes.ts` (duplicate merge #159/#193, stale flagging #138, junk cleanup #195, bio audit #142, completeness recalibration #140), closing it when its acceptance criteria are met. Non-automatable top issues are logged for the next agent session. One fix per cycle, 5-min backoff, max 6 cycles, then switches to the scrape phase.
