...
export function OptOutForm() {
  ...
  return (
    <div className="mx-auto max-w-md space-y-4">
      ...
      <button
        onClick={submit}
        className="w-full rounded-lg border border-accent bg-accent/10 px-4 py-3 font-mono text-sm font-semibold text-foreground transition-colors hover:bg-accent/20"
      >
        Remove my profile
      </button>
      {status === "done" && (
        <p className="font-mono text-xs text-emerald-400">
          Done. Your profile is hidden from the directory.
        </p>
      )}
      {status === "error" && (
        <p className="font-mono text-xs text-red-400">
          Could not process that id. Check it and try again.
        </p>
      )}
    </div>
  );
