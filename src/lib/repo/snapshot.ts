...
function sortDjs(rows: DjRow[], sort?: string): DjRow[] {
  switch (sort) {
    ...
    case "updated":
      return [...rows].sort(
        (a, b) =>
          String(b.updated_at).localeCompare(String(a.updated_at)) || a.name.localeCompare(b.name)
      );
    case "gigs":
      return [...rows].sort(
        (a, b) => b.upcoming_events - a.upcoming_events || a.name.localeCompare(b.name)
      );
    case "completeness":
    default:
      return [...rows].sort(
        (a, b) => b.data_completeness - a.data_completeness || a.name.localeCompare(b.name)
      );
  }
}
...
export class SnapshotRepo implements DataRepository {
  async listDjs(opts: DjQueryOptions = {}): Promise<DjRow[]> {
    ...
    if (opts.query) {
      ...
      rows = rows.filter((dj) =>
        `${dj.name} ${dj.bio ?? ""} ${dj.genres.join(" ")}`.toLowerCase().includes(q)
      );
    }
  ...
  async getEvents(opts: EventQueryOptions = {}): Promise<EventRow[]> {
    ...
    if (opts.venue)
      rows = rows.filter((event) => event.venue?.toLowerCase() === opts.venue!.toLowerCase());
  ...
  async getDjGigs(djId: string, limit = 20): Promise<EventRow[]> {
    ...
    const djEventIds = new Set(
      ((snapshot.eventDjs ?? []) as EventDjLink[])
    ...
    return (snapshot.events as EventRow[])
      .filter(
        (event) =>
          djEventIds.has(event.id) && event.starts_at && new Date(event.starts_at).getTime() > now
      )
      .sort((a, b) => new Date(a.starts_at).getTime() - new Date(b.starts_at).getTime())
      .slice(0, limit);
  }
  ...
  async getDjMixes(djId: string): Promise<MixRow[]> {
    return (
      (snapshot.mixes as MixRow[] | undefined)?.filter(
        (mix) => mix.dj_id === djId && mix.status !== "dead"
      ) ?? []
    );
  }
  ...
  async getDjReleases(djId: string): Promise<ReleaseRow[]> {
    return (
      (snapshot.releases as ReleaseRow[] | undefined)?.filter(
        (release) => release.dj_id === djId
      ) ?? []
    );
  }
  ...
  async getDjArticles(djId: string): Promise<ArticleRow[]> {
    return (
      (snapshot.articles as ArticleRow[] | undefined)?.filter(
        (article) => article.dj_id === djId
      ) ?? []
    );
  }
  ...
  async getDjCollabs(djId: string): Promise<CollabRow[]> {
    ...
    const knownByName = new Map(
      (snapshot.djs as DjRow[]).map((row) => [row.name.toLowerCase(), row.id])
    );
  ...
  async getVenuesWithCounts(): Promise<VenueWithCounts[]> {
    ...
    return (snapshot.venues as VenueRow[]).map((venue) => ({
      ...
      upcoming_events: (snapshot.events as EventRow[]).filter(
        (event) =>
          event.venue?.toLowerCase() === venue.name.toLowerCase() &&
          event.starts_at &&
          new Date(event.starts_at).getTime() > now
      ).length,
    }));
  ...
  async getEventLineup(eventId: string): Promise<DjRow[]> {
    ...
    const ids = ((snapshot.eventDjs ?? []) as EventDjLink[])
