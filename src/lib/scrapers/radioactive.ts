...
export function extractPresenters(
  shows: Array<{ title: string; url: string }>
): Array<{ name: string; show: string; url: string }> {
  ...
  async run(pool: Pool): Promise<ScrapeResult> {
    ...
    if (presenters.length === 0) {
      return {
        ...
        items_found: 0,
        items_new: 0,
        error: "Fetched but no presenter names extracted",
      };
    }
    const existing = new Set(
      (await pool.query("SELECT lower(name) AS name FROM djs")).rows.map(
