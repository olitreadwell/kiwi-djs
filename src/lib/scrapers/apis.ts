...
export async function enrichMusicbrainz(pool: Pool, dj: DjRow): Promise<ScrapeResult> {
  ...
  if (!full)
    return { status: "partial", items_found: 0, items_new: 0, error: "MusicBrainz lookup empty" };
  ...
  for (const relation of full.relations ?? []) {
    ...
    if (column) {
      if (column === "soundcloud_url") {
        const existing = (await pool.query(`SELECT soundcloud_url FROM djs WHERE id = $1`, [dj.id]))
          .rows[0]?.soundcloud_url as string | undefined;
        if (
          !existing ||
          (!isCleanSoundcloudPermalink(existing) && isCleanSoundcloudPermalink(resource))
        ) {
...
export async function enrichItunes(pool: Pool, dj: DjRow): Promise<ScrapeResult> {
  ...
  if (artist.artworkUrl100) {
    ...
    await pool.query(`UPDATE djs SET image_url = COALESCE(image_url, $2) WHERE id = $1`, [
      dj.id,
      image,
    ]);
    found += 1;
...
export async function enrichVenueRegions(pool: Pool): Promise<ScrapeResult> {
  const venues = (
    await pool.query(
      `SELECT id, name, address FROM venues WHERE address IS NOT NULL AND (region IS NULL OR region = '')`
    )
  ...
  for (const venue of venues) {
    ...
    try {
      ...
      const region =
        results[0]?.address?.state ?? results[0]?.address?.region ?? results[0]?.address?.city;
      if (region) {
      ...
      console.log(
        `  enrich-venue-regions: ${venue.name} → error (${err instanceof Error ? err.message : String(err)})`
      );
    }
    await sleep(1000);
  }
...
const NZ_SIGNAL =
  /\bnew zealand\b|aotearoa|\bnz\b|auckland|wellington|christchurch|dunedin|hamilton|tauranga|queenstown|nelson|napier|rotorua|palmerston north|new plymouth|whanganui|gisborne|timaru|invercargill|whangarei|hastings|lower hutt|upper hutt|porirua|taupo|wanaka|blenheim|waiheke/i;
...
function wikipediaExtractIsRelevant(djName: string, title: string, extract: string): boolean {
  ...
  if (
    /\balbum by\b|\bis (?:the |a |an )?(?:debut |self-titled |eponymous )?(?:studio |compilation |live )?album\b|\brecord label\b|\bimprint\b|\bcompany\b|\bbusiness\b|\bbrand\b/i.test(
      haystack
    )
  )
    return false;
  return /\b(dj|deejay|disc jockey|musician|band|singer|songwriter|producer|rapper|group|drum and bass|dnb|electronic music|house music|techno|reggae|dub)\b/i.test(
    haystack
  );
}

export async function enrichBio(pool: Pool, dj: DjRow): Promise<ScrapeResult> {
  const existing = (await pool.query("SELECT bio FROM djs WHERE id = $1", [dj.id])).rows[0]?.bio as
    string | undefined;
  if (existing)
    return { status: "partial", items_found: 0, items_new: 0, error: "Bio already present" };
  ...
  try {
    ...
    if (artist) {
      await sleep(1000);
      const full = await musicbrainzLookup(artist.id);
      const wikiRelation = (full?.relations ?? []).find(
        (relation) => relation.type === "wikipedia"
      );
      const wikiTitle = wikiRelation?.url?.resource
        ? wikipediaTitleFromUrl(wikiRelation.url.resource)
        : null;
      if (wikiTitle) {
  ...
  const mixcloud = (
    await pool.query(`SELECT url FROM dj_links WHERE dj_id = $1 AND type = 'mixcloud' LIMIT 1`, [
      dj.id,
    ])
  ).rows[0]?.url as string | undefined;
  if (mixcloud) {
