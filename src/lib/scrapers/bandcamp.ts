...
export async function enrichBandcamp(pool: Pool, dj: DjRow): Promise<ScrapeResult> {
  const link = await pool.query(
    `SELECT url FROM dj_links WHERE dj_id = $1 AND type = 'bandcamp' LIMIT 1`,
    [dj.id]
  );
  ...
  const locationTag = tags.find(
    (tag) => isNzLocation(tag) || /\bnew zealand\b|aotearoa|\bnz\b/i.test(tag)
  );
  if (locationTag) {
    await pool.query(
      `UPDATE djs SET profile_location = COALESCE(profile_location, $2) WHERE id = $1`,
      [dj.id, `Bandcamp: ${locationTag}`]
    );
    await pool.query(
  ...
  const genreTags = tags.filter(
    (tag) =>
      !isNzLocation(tag) && !/\bnew zealand\b|aotearoa|\bnz\b/i.test(tag) && !/^www\./i.test(tag)
  );
