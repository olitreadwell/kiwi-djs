...
const MIX_KEYWORDS =
  /\b(mix|set|live|dj set|studio mix|guest mix|boiler room|radio show|podcast|mashup|remix)\b/i;
...
export async function discoverYoutubeMixes(pool: Pool): Promise<ScrapeResult> {
  ...
  if (!key) {
    return {
      ...
      items_found: 0,
      items_new: 0,
      error: "no YouTube credentials (set YOUTUBE_API_KEY)",
    };
  }
  ...
  for (const row of djs.rows) {
    ...
    try {
      ...
      for (const item of body.items ?? []) {
        ...
        if (!title.toLowerCase().includes(djName.toLowerCase()) || !MIX_KEYWORDS.test(title))
          continue;
      ...
      if (firstVideo?.id?.videoId) {
        ...
        const videoRes = await fetch(`${API}/videos?${videoQuery}`, {
        ...
        if (videoRes.ok) {
          ...
          if (
            channelId &&
            channelTitle &&
            normalizeArtistName(channelTitle) === normalizeArtistName(djName)
          ) {
            await upsertDjLink(
              pool,
              djId,
      ...
      console.log(
        `  discover-youtube: ${djName} → error (${err instanceof Error ? err.message : String(err)})`
      );
    }
    await sleep(500);
  }
