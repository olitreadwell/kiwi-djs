...
export function parseSanFranHtml(
  html: string
): Array<{ id: string; name: string; startsAt: Date | null; url: string }> {
  ...
  $('a[href*="/all-events/"]').each((_, el) => {
    ...
    events.push({
      ...
      name,
      startsAt,
      url: `https://sanfran.co.nz${href.startsWith("/") ? href : `/${href}`}`,
    });
  });
  return events;
}
...
export const sanfranScraper: Scraper = {
  ...
  async run(pool: Pool): Promise<ScrapeResult> {
    ...
    return {
      ...
      items_new: newCount,
      error: events.length === 0 ? "No events parsed" : undefined,
    };
  },
};
