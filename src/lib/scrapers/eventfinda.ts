...
async function scrapeSitemap(pool: Pool): Promise<ScrapeResult> {
  ...
  const urls = [
    ...sitemap.matchAll(/<loc>(https:\/\/www\.eventfinda\.co\.nz\/\d{4}\/[^<]+)<\/loc>/g),
  ]
    .map((match) => match[1])
  ...
  for (const url of urls) {
    try {
      ...
      const startsAt = datetime
        ? new Date(datetime.replace(/&ndash;.*$/, "").replace(",", ""))
        : null;
      ...
      await linkDjToEvent(
        pool,
        `eventfinda-sitemap-${createHash("sha1").update(url).digest("hex").slice(0, 12)}`,
        name
      );
      found += 1;
    } catch (err) {
      console.log(
        `  eventfinda-sitemap: ${url} → error (${err instanceof Error ? err.message : String(err)})`
      );
    }
    await sleep(500);
  }
  return {
    ...
    items_found: found,
    items_new: newCount,
    error: found === 0 ? "No sitemap events parsed" : undefined,
  };
}
