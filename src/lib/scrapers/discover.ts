...
export function isJunkName(name: string): boolean {
  ...
  return [...JUNK_NAMES].some(
    (junk) =>
      normalized === junk || normalized.startsWith(`${junk} `) || normalized.endsWith(` ${junk}`)
  );
}
...
export async function discoverFromEvents(pool: Pool): Promise<ScrapeResult> {
  const events = (await pool.query("SELECT id, name FROM events WHERE dj_id IS NULL"))
    .rows as Array<{ id: string; name: string }>;
  const existing = await loadExistingNames(pool);
  const venueNames = new Set(
    (await pool.query("SELECT name FROM venues")).rows.map((row) =>
      normalizeArtistName(row.name as string)
    )
  );
  let found = 0;
  let newCount = 0;
  ...
  return {
    ...
    items_found: found,
    items_new: newCount,
    error: found === 0 ? "No new names" : undefined,
  };
}
...
export async function discoverSoundcloudNz(pool: Pool): Promise<ScrapeResult> {
  ...
  if (!cid)
    return { status: "error", items_found: 0, items_new: 0, error: "no SoundCloud client id" };
  const queries = [
    "new zealand",
    "wellington",
    "auckland",
  ...
  for (const query of queries) {
    try {
      const res = await fetch(
        `https://api-v2.soundcloud.com/search/users?q=${encodeURIComponent(query)}&client_id=${cid}&limit=50`,
        {
...
function musicbrainzJson(url: string): Promise<{
  count: number;
  artists: Array<{
    id: string;
    name: string;
  ...
  return new Promise((resolve, reject) => {
    const req = httpsGet(
      url,
      { headers: { "user-agent": MB_UA, accept: "application/json" }, family: 4, timeout: 15000 },
      (res) => {
        const chunks: Buffer[] = [];
...
export async function discoverMusicbrainzNzEdm(pool: Pool): Promise<ScrapeResult> {
  ...
  if (cid) {
    ...
    for (const candidate of candidates) {
      try {
        const res = await fetch(
          `https://api-v2.soundcloud.com/search/users?q=${encodeURIComponent(candidate.name)}&client_id=${cid}&limit=5`,
          {
            headers: { accept: "application/json" },
            signal: AbortSignal.timeout(15000),
          }
        ...
        await upsertDjLink(
          ...
          `SoundCloud: ${match.username}`,
          match.followers_count,
          match.track_count
        );
        await pool.query(
          `UPDATE djs SET soundcloud_url = $2, image_url = COALESCE(image_url, $3), verification_sources = ARRAY['location','links'] WHERE id = $1`,
          [candidate.id, `https://soundcloud.com/${match.permalink}`, match.avatar_url ?? null]
        );
      } catch {
        // keep going
      }
...
export async function loadExistingNames(pool: Pool): Promise<Set<string>> {
  const names = (await pool.query("SELECT lower(name) AS name FROM djs")).rows.map(
    (row) => row.name as string
  );
  const aliases = (await pool.query("SELECT alias FROM dj_aliases")).rows.map(
    (row) => row.alias as string
  );
...
function nameContainsCityWord(name: string): boolean {
  const normalized = normalizeArtistName(name);
  return CITY_WORDS.some(
    (word) =>
      normalized === word || normalized.includes(`${word} `) || normalized.includes(` ${word}`)
  );
}
...
export async function discoverFromMixcloud(pool: Pool): Promise<ScrapeResult> {
  ...
  for (const query of queries) {
    ...
    for (const cloudcast of data.data ?? []) {
      ...
      const anchored = WELLINGTON_ANCHORS.some((anchor) =>
        cloudcastName.includes(normalizeArtistName(anchor))
      );
      ...
      if (result.rows.length > 0) {
        ...
        if (junk)
          console.log(`  discover-mixcloud: junk candidate skipped from promotion: ${user.name}`);
      }
      found += 1;
    }
    await sleep(500);
  ...
  return {
    ...
    items_found: found,
    items_new: newCount,
    error: found === 0 ? "No Wellington-anchored cloudcasts" : undefined,
  };
}
...
export async function verifyDiscovered(pool: Pool): Promise<ScrapeResult> {
  ...
  const series = (
    await pool.query(
      `SELECT id, name FROM djs WHERE opt_out = FALSE AND (discovery_note IS NULL OR discovery_note <> 'junk')`
    )
  ).rows as Array<{ id: string; name: string }>;
  let parked = 0;
  for (const dj of series) {
    ...
    await pool.query(
      `UPDATE djs SET active = FALSE, discovery_note = 'junk', verification_level = 0, updated_at = now() WHERE id = $1`,
      [dj.id]
    );
    parked += 1;
  }
...
function extractDjNamesFromTitle(title: string): string[] {
  ...
  const push = (raw: string): void => {
    ...
    if (
      /^(the|a|an|this|these|those|how|why|what|when|where|region|next|month|tomorrow|today|night|weekend|mural|flood|relief|larger|from|new|top|best|all|cases|artist|band|crew|collective|show|gig|party|festival)\b/i.test(
        name
      )
    )
      return;
...
const NEWS_ANCHOR_QUERIES = [
  '"wellington" dj interview',
  '"wellington" dj news',
  '"wellington" dj preview',
  '"wellington" djs',
  '"pōneke" dj',
  '"te whanganui-a-tara" dj',
  '"wellington" techno',
  '"wellington" drum and bass',
  '"wellington" house music',
  '"wellington" nightlife dj',
  '"wellington" club dj',
  '"san fran" dj wellington',
  '"meow" wellington dj',
  '"cuba street" dj',
  '"121 festival" dj',
  '"valhalla" wellington dj',
  '"muzic.net.nz" dj',
  '"nz musician" dj',
  '"rnz" dj interview',
  '"spinoff" dj wellington',
  '"sniffers" dj',
  '"ambient light" dj',
  '"cheeky monkey" dj wellington',
  '"backseat mafia" dj nz',
];
...
export async function discoverFromNewsArticles(pool: Pool): Promise<ScrapeResult> {
  ...
  return {
    ...
    items_found: found,
    items_new: newCount,
    error: found === 0 ? "No DJ names in news" : undefined,
  };
}
...
export async function discoverFromNzMusicFeeds(pool: Pool): Promise<ScrapeResult> {
  ...
  return {
    ...
    items_found: found,
    items_new: newCount,
    error: found === 0 ? "No DJ names in NZ music feeds" : undefined,
  };
}
