...
export async function discoverSpotifyNzEdm(pool: Pool): Promise<ScrapeResult> {
  ...
  if (!token) {
    return {
      ...
      items_found: 0,
      items_new: 0,
      error: "no Spotify credentials (set SPOTIFY_CLIENT_ID / SPOTIFY_CLIENT_SECRET)",
    };
  }
  ...
  if (chartArtists.size === 0) {
    return {
      ...
      items_found: 0,
      items_new: 0,
      error: "Spotify NZ Top 50 chart returned no artists",
    };
  }
  ...
  const artists = (artistsResponse?.artists ?? [])
    .filter(
      (artist) =>
        artist.name &&
        artist.genres.some((genre) => EDM_GENRES.has(normaliseGenres([genre])[0] ?? genre))
    )
