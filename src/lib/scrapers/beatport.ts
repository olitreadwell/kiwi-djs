...
export async function enrichBeatport(pool: Pool, dj: DjRow): Promise<ScrapeResult> {
  const link = await pool.query(
    `SELECT url FROM dj_links WHERE dj_id = $1 AND type = 'beatport' LIMIT 1`,
    [dj.id]
  );
  ...
  try {
    ...
    if (!res.ok)
      return {
        ...
        items_found: 0,
        items_new: 0,
        error: `Beatport HTTP ${res.status}`,
      };
    text = await res.text();
  } catch (err) {
    return {
  ...
  const navGenres = new Set(
    [...text.matchAll(/\[([A-Za-z0-9 &/]+)\]\(https:\/\/www\.beatport\.com\/genre\//g)].map(
      (match) => norm(match[1])
    )
  );
  ...
  if (counts.size === 0)
    return { status: "partial", items_found: 0, items_new: 0, error: "No genre filters found" };
  ...
  const merged = [...normalised, ...existing.filter((genre) => !normalised.includes(genre))].slice(
    0,
    8
  );
