...
export async function discoverLastfmNz(pool: Pool): Promise<ScrapeResult> {
  ...
  if (!key) {
    return {
      ...
      items_found: 0,
      items_new: 0,
      error: "no Last.fm credentials (set LASTFM_API_KEY)",
    };
  }
  const top = (await lastfmJson(key, {
    method: "geo.gettopartists",
    country: "New Zealand",
    limit: "50",
  })) as { topartists?: { artist?: LastfmArtist[] } } | null;
  ...
  if (artists.length === 0) {
    return {
      ...
      items_found: 0,
      items_new: 0,
      error: "Last.fm NZ top artists returned nothing",
    };
  }
  ...
  for (const artist of artists.slice(0, 15)) {
    try {
      ...
      const tags = (await lastfmJson(key, {
        method: "artist.getTopTags",
        artist: artist.name,
      })) as { toptags?: { tag?: Array<{ name: string }> } } | null;
      const rawTags = (tags?.toptags?.tag ?? []).map((tag) => tag.name);
      const genres = normaliseGenres(rawTags)
        .filter((genre) => KNOWN_ELECTRONIC_GENRES.has(genre))
        .slice(0, 8);
      ...
      console.log(
        `  discover-lastfm-nz: ${artist.name} → error (${err instanceof Error ? err.message : String(err)})`
      );
    }
    await sleep(500);
  }
