  ...
  if (!isDbMode) {
    return (
      <p className="p-8 font-mono text-sm text-muted">
        Suggestions require DATABASE_URL to be configured.
      </p>
    );
  }
  const pool = getPool();
          ...
          <li key={row.id} className="rounded-lg border border-edge bg-surface p-4">
            <div className="flex items-center justify-between gap-4">
              <p className="text-sm font-semibold text-foreground">
                {row.dj_name ?? "unknown DJ"} · {row.field}
              </p>
              <span
                className={`font-mono text-[11px] ${row.status === "pending" ? "text-accent" : "text-muted"}`}
              >
                {row.status}
              </span>
            </div>
            {row.current_value && (
              <p className="mt-2 text-sm text-muted">was: {row.current_value}</p>
            )}
