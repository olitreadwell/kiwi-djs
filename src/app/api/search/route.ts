...
export async function POST(request: Request) {
  ...
  if (!isDbMode) {
    const results = (
      snapshot.djs as Array<{ name: string; bio: string | null; genres: string[] }>
    ).filter((dj) =>
      `${dj.name} ${dj.bio ?? ""} ${dj.genres.join(" ")}`
        .toLowerCase()
  ...
  await pool.query(`INSERT INTO search_events (query, results) VALUES ($1, $2)`, [
    query,
