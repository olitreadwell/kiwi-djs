...
export async function POST(request: Request) {
  ...
  if (!isDbMode) {
    return NextResponse.json(
      { ok: false, error: "Opt-out requires DATABASE_URL to be configured", mode: "snapshot" },
      { status: 503 }
    );
  }
  const pool = getPool();
  const result = await pool.query(`UPDATE djs SET opt_out = TRUE WHERE id = $1 RETURNING id`, [
    djId,
  ]);
