...
export default async function DiscoverPage({
  ...
  const inCity = cityFilter
    ? all.filter((dj) => {
        const city =
          cityFromLocation(dj.profile_location ?? null) ??
          (dj.city && dj.city !== "" ? dj.city : null);
  ...
  for (const dj of all) {
    const city =
      cityFromLocation(dj.profile_location ?? null) ?? (dj.city && dj.city !== "" ? dj.city : null);
    if (city) cityCounts.set(city, (cityCounts.get(city) ?? 0) + 1);
  }
  const cities = [...cityCounts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
  const fullProfiles = [...all]
    .sort(
      (a, b) =>
        (profileTier(a) === "tier1" ? -1 : 1) - (profileTier(b) === "tier1" ? -1 : 1) ||
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <p className="mt-2 font-mono text-xs text-muted">
        {cityFilter
          ? `DJs in ${cities.find(([name]) => name.toLowerCase() === cityFilter.toLowerCase())?.[0] ?? cityFilter}`
          : "Browse the directory by sound or city"}
      </p>

      {cityFilter && (
        <>
          <h2 className="mt-10 text-xl font-bold text-foreground">
            {cities.find(([name]) => name.toLowerCase() === cityFilter.toLowerCase())?.[0] ??
              cityFilter}
          </h2>
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {inCity.slice(0, 12).map((dj) => (
      ...
      <p className="mt-1 font-mono text-xs text-muted">
        The most complete dossiers — bio, mixes, gigs, sources and all.
      </p>
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {fullProfiles.map((dj) => (
          <DjCard key={dj.id} dj={dj} />
