...
export default async function DjLinksPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-12">
      <Link href={`/djs/${dj.id}`} className="font-mono text-xs text-muted hover:text-accent">
        ← {dj.name}
      </Link>
      <h1 className="mt-4 text-3xl font-black text-foreground">Links</h1>
      <p className="mt-2 font-mono text-xs text-muted">
        {links.length} public link{links.length === 1 ? "" : "s"} for {dj.name}. The flagged one is
        the best guess — vote on which is right.
      </p>
                    ...
                    <div className="min-w-0">
                      ...
                      {(link.followers > 0 || link.track_count > 0) && (
                        <span className="ml-2 font-mono text-xs text-faint">
                          {link.followers > 0
                            ? `${link.followers.toLocaleString("en-NZ")} followers`
                            : ""}
                          {link.followers > 0 && link.track_count > 0 ? " · " : ""}
                          {link.track_count > 0
                            ? `${link.track_count} mix${link.track_count === 1 ? "" : "es"}`
                            : ""}
                        </span>
                      )}
                      {bestById.has(link.id) && (
                        <span className="ml-2 font-mono text-xs text-accent">best guess</span>
                      )}
                    </div>
                    <LinkFeedback
                      linkId={link.id}
                      helpful={link.helpful}
                      unhelpful={link.unhelpful}
                    />
                  </li>
                ))}
              </ul>
