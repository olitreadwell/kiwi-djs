...
`;
  ...
  log(`Wrote ${plistPath}`);
  const boot = run("launchctl", [
    "bootstrap",
    `gui/${process.getuid?.() ?? process.env.UID ?? ""}`,
    plistPath,
  ]);
  log(
    boot.ok
      ? `launchctl bootstrap ok: ${boot.out}`
      : `launchctl bootstrap failed: ${boot.out} — run: launchctl bootstrap gui/$(id -u) ${plistPath}`
  );
}

function regenerateSnapshot(): void {
  const { ok, out } = run(process.execPath, [
    "--env-file=.env.local",
    "--import",
    "tsx",
...
function logQueueTop(queue: QueueIssue[], count = 3): void {
  ...
  const lines = queue
    ...
    .map(
      (issue, i) =>
        `  ${i + 1}. #${issue.number} ${issue.title}${issue.workable ? " (automatable)" : ""}`
    );
...
async function runIssuesCycle(
  pool: ReturnType<typeof getPool>
): Promise<{ switchedToScrape: boolean }> {
  ...
  if (!top.workable) {
    log(
      `Issues phase: top issue #${top.number} — "${top.title}" (${top.labels.join(", ") || "unlabelled"}) needs an agent session. Queue written to .loop/queue.json.`
    );
  }
  ...
  log(
    `Issues phase (cycle ${phase.cyclesInPhase + 1}): working on #${fix.issueNumber} — ${fix.title}.`
  );
  try {
    ...
    log(
      `Issues phase: #${fix.issueNumber} fix errored: ${err instanceof Error ? err.message : String(err)}`
    );
  }
...
async function auditAndFileIssues(pool: ReturnType<typeof getPool>): Promise<void> {
  ...
  for (const row of failing) {
    file(
      `fix: scraper "${row.source}" failing`,
      `${row.source}: ${row.failures} errors in 48h. See scrapes table.`
    );
  }
  ...
  if (nonNzLocation.length >= 2) {
    file(
      ...
      `${nonNzLocation
        .slice(0, 5)
        .map((d) => `- ${d.name} (${d.profile_location})`)
...
async function writeHandoff(
  pool: ReturnType<typeof getPool>,
  totals: { totalNew: number; totalFound: number }
): Promise<void> {
  ...
  const handoff = [
    ...
    failing.length > 0
      ? `Failing sources: ${failing.map((f) => f.source).join(", ")}`
      : "Failing sources: none",
    top
      ? `Next issue: #${top.number} — ${top.title}${top.workable ? " (automatable)" : " (needs agent)"}`
      : "Next issue: none open",
...
async function compactDataset(pool: ReturnType<typeof getPool>): Promise<void> {
  ...
  const scrapes = await pool.query(
    `DELETE FROM scrapes WHERE started_at < now() - interval '30 days' RETURNING id`
  );
  ...
  const genreRows = (await pool.query(`SELECT id, genres FROM djs WHERE cardinality(genres) > 0`))
    .rows as Array<{
    id: string;
    genres: string[];
  }>;
  ...
  log(
    `Compacted: ${junk.rows.length} junk candidates, ${scrapes.rows.length} stale scrape rows, ${genreFixes} genre normalisations, ${merged} duplicate events merged (${deleted} deleted), ${articleFixes} irrelevant articles removed.`
  );
}

async function runCycle(
  pool: ReturnType<typeof getPool>
): Promise<{ totalNew: number; totalFound: number }> {
  ...
  if (disabled.size > 0) {
    log(
      `Skipping disabled sources (${DISABLE_AFTER_FAILURES}+ consecutive errors): ${[...disabled].join(", ")}.`
    );
  }
  ...
  for (const result of results) {
    log(
      `${result.status.padEnd(7)} found=${result.items_found} new=${result.items_new}${result.error ? ` — ${result.error}` : ""}`
    );
  }
    ...
    do {
      ...
      if (DATASET_FIXES.some((fix) => open.has(fix.issueNumber))) {
        ...
        savePhase({
          phase: "issues",
          cyclesInPhase: 0,
          issuesResolved: loadPhase().issuesResolved,
        });
        continue;
      }
