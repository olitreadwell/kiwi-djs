...
async function main(): Promise<void> {
  ...
  if (!consumerKey || !consumerSecret) {
    console.error(
      "Set DISCOGS_CONSUMER_KEY and DISCOGS_CONSUMER_SECRET first (already in .env.local)."
    );
    process.exit(1);
  }
  ...
  console.log(
    `token prefix: ${accessToken.slice(0, 6)}… | secret prefix: ${accessTokenSecret.slice(0, 6)}…`
  );
}
