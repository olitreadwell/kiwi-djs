...
async function main() {
  ...
  check(
    ...
    "spec declares parameterized path"
  );

  // Resolve {id} parameters against a real DJ id: the snapshot dataset
  // changes daily, so a hardcoded id eventually 404s.
  const listRes = await fetch(`${base}/api/v1/djs`);
  const list = await listRes.json();
  const sampleDjId = list.data?.[0]?.id ?? "test";
  ...
  for (const path of paths) {
    ...
    for (const method of methods) {
      ...
      for (const param of operation.parameters ?? []) {
        if (param.in === "path") {
          const value = path.includes("{id}") ? encodeURIComponent(sampleDjId) : "test";
  ...
  check(
    djs.data.every((dj) => typeof dj.id === "string" && typeof dj.name === "string"),
    "every DJ has id + name"
  );

  const detailRes = await fetch(`${base}/api/v1/djs/${encodeURIComponent(sampleDjId)}`);
  ...
  check(
    dataset.djs?.length === djs.meta.total,
    `dataset.djs (${dataset.djs?.length}) == /djs total (${djs.meta.total})`
  );
  ...
  check(
    csvLines.length - 1 === djs.meta.total,
    `CSV rows (${csvLines.length - 1}) == djs total (${djs.meta.total})`
  );
  ...
  check(
    metaRes.status === 200 && typeof meta.version === "string",
    "GET /api/v1/dataset/meta returns version"
  );
  check(
    meta.counts?.djs === djs.meta.total,
    `dataset/meta counts.djs (${meta.counts?.djs}) == /djs total (${djs.meta.total})`
  );
